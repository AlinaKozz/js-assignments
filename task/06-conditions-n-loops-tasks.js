...
function isInsideCircle(circle, point) {
    return Math.hypot(point.x - circle.center.x, point.y - circle.center.y) < circle.radius ? true : false;
}
...
function isBracketsBalanced(str) {
    ...
    for (var i = 0; i < str.length; i++) {
        var brackets = str[i];
        if (open.indexOf(brackets) !== -1) {
            if (stack[stack.length - 1] !== close.indexOf(brackets)) {
                stack.push(open.indexOf(brackets));
                continue;
            }
        }
        if (close.indexOf(brackets) !== -1) {
            var pop = stack.pop();
            if (close.indexOf(brackets) !== pop) {
                return false;
            }
        }
    }
